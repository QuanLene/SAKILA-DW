...
CREATE TABLE dim_customer (
    customer_key INT NOT NULL IDENTITY PRIMARY KEY,
...
alter column active NVARCHAR(50);

CREATE TABLE dim_store (
    store_key INT NOT NULL IDENTITY PRIMARY KEY,       -- surrogate key
    store_id INT NULL,                                 -- mã cửa hàng gốc
    store_name NVARCHAR(100) NULL,                     -- tên cửa hàng
    manager_name NVARCHAR(100) NULL,                   -- tên người quản lý
    ...
    date_from DATETIME NULL,                           -- ngày bắt đầu hiệu lực
    date_to DATETIME NULL                              -- ngày kết thúc hiệu lực
);
CREATE TABLE dim_staff (
    staff_key INT NOT NULL IDENTITY PRIMARY KEY,
    staff_id INT NULL,
    first_name NVARCHAR(50) NULL,
    last_name NVARCHAR(50) NULL,
    full_name AS (first_name + ' ' + last_name),
...
CREATE TABLE dim_product (
    product_key INT NOT NULL IDENTITY PRIMARY KEY,
    film_id INT NULL,
...
CREATE TABLE fact_rental (
    rental_key INT NOT NULL IDENTITY PRIMARY KEY,        -- Khóa chính của bảng fact
    rental_id INT NULL,                                  -- ID giao dịch thuê trong hệ thống nguồn
    
    -- Khóa ngoại liên kết đến các bảng chiều
    customer_key INT NOT NULL,                           -- Khách hàng
    staff_key INT NOT NULL,                              -- Nhân viên
    store_key INT NOT NULL,                              -- Cửa hàng
    product_key INT NOT NULL,                            -- Sản phẩm (phim)
    rental_date_key INT NOT NULL,                        -- Ngày thuê
    return_date_key INT NULL,                            -- Ngày trả phim
    payment_date_key INT NULL,                           -- Ngày thanh toán
    
    -- Các thông tin định lượng
    rental_count INT DEFAULT 1,                          -- Số lượng phim được thuê (thường là 1)
    rental_amount FLOAT NULL,                            -- Số tiền thuê phim
    rental_duration INT NULL,                            -- Thời gian thuê (số ngày)
    late_fee FLOAT NULL,                                 -- Phí phạt trả trễ (nếu có)
    
    -- Dấu thời gian ghi nhận dữ liệu
    date_from DATETIME NULL,
    date_to DATETIME NULL
);
...
INSERT INTO dim_date (date_key, full_date, [day], [month], [quarter], [year], [week_of_year], [day_name], [month_name], [is_weekend], [is_holiday])
SELECT 
    ...
    0 AS is_holiday   -- Mặc định chưa đánh dấu ngày lễ
FROM DateSeries
